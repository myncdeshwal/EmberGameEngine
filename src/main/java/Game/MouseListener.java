...
public class MouseListener {
    ...
    public static Vector2f gameViewportSize = new Vector2f();
    ...
    public static void mousePosCallback(long window, double xpos, double ypos) {
        if (get().mouseButtonDown > 0 && ( (Math.abs(get().xPos) -xpos)>0 || Math.abs(get().yPos -ypos)>0 ) ) {
            get().isDragging = true;
        }
    ...
    public static void setGameViewportSize(Vector2f vector2f) {
        MouseListener.gameViewportPos = vector2f;
    }

    public static void setGameViewportPos(Vector2f vector2f) {
        MouseListener.gameViewportSize = vector2f;
    }

    public static float getScreenX() {
        float currentX = getX() - get().gameViewportPos.x;
        currentX = (currentX / get().gameViewportSize.x) * 1920.f;
        return currentX;
    }

    public static float getScreenY() {
        float currentY = getY() - get().gameViewportPos.y;
        currentY = 2160.0f - ((currentY / get().gameViewportSize.y) * 1088.f);
        return currentY;
    }
